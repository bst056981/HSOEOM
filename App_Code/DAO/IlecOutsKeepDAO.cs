...
namespace Agile.DAO {
    public class IlecOutsKeepDAO {

        public IlecOutsKeepDAO() {

        }

        public IlecOutsKeep Select(string p) {
            IlecOutsKeep q = new IlecOutsKeep();
            DataTable dt = DBHelper.SelectDataTable(IlecOutsKeep.SELECT_ILEC_OUTS_KEEP, DBHelper.mp("ILEC_OUTS_ID", p));
        ...
        public void Insert(IlecOutsKeep p) {
            ...
            DBHelper.Execute(IlecOutsKeep.INSERT_ILEC_OUTS_KEEP, paramsList);
        }

        public void Update(IlecOutsKeep p) {
            OracleParameter[] paramsList = createParamList(p);
            DBHelper.Execute(IlecOutsKeep.UPDATE_ILEC_OUTS_KEEP, paramsList);
        }

        public void Delete(string p) {
            DBHelper.Execute(IlecOutsKeep.DELETE_ILEC_OUTS_KEEP, DBHelper.mp("ILEC_OUTS_ID", p));
        }

        private OracleParameter[] createParamList(IlecOutsKeep p) {
            int cntr = 0;
