...
namespace Agile.DAO {
    public class ClecOutsKeepDAO {

        public ClecOutsKeepDAO() {

        }

        public ClecOutsKeep Select(string p) {
            ClecOutsKeep q = new ClecOutsKeep();
            DataTable dt = DBHelper.SelectDataTable(ClecOutsKeep.SELECT_CLEC_OUTS_KEEP, DBHelper.mp("CLEC_OUTS_ID", p));
        ...
        public void Insert(ClecOutsKeep p) {
            ...
            DBHelper.Execute(ClecOutsKeep.INSERT_CLEC_OUTS_KEEP, paramsList);
        }

        public void Update(ClecOutsKeep p) {
            OracleParameter[] paramsList = createParamList(p);
            DBHelper.Execute(ClecOutsKeep.UPDATE_CLEC_OUTS_KEEP, paramsList);
        }

        public void Delete(string p) {
            DBHelper.Execute(ClecOutsKeep.DELETE_CLEC_OUTS_KEEP, DBHelper.mp("CLEC_OUTS_ID", p));
        }

        private OracleParameter[] createParamList(ClecOutsKeep p) {
            int cntr = 0;
