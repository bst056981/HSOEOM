...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
        ...
        <tr>
            ...
            <td>
                ...
                <asp:SqlDataSource ID="dsDealer" runat="server" ConnectionString="<%$ ConnectionStrings:ConnectionString %>"
                    ProviderName="<%$ ConnectionStrings:ConnectionString.ProviderName %>" SelectCommand="SELECT DISTINCT DEALER FROM CUSTOMER WHERE (:STATUS = 'Active' AND INACTIVE_DATE IS NULL) OR (:STATUS = 'Inactive' AND INACTIVE_DATE != ' ') ORDER BY DEALER">
                    <SelectParameters>
    ...
    <asp:SqlDataSource ID="dsCustomer" runat="server" ConnectionString="<%$ ConnectionStrings:ConnectionString %>"
        ProviderName="<%$ ConnectionStrings:ConnectionString.ProviderName %>" SelectCommand="SELECT CUST_ID, DICE, NAME, DEALER,  START_DATE, INACTIVE_DATE FROM CUSTOMER WHERE (( TO_DATE(START_DATE, 'MM/DD/YYYY') >= TO_DATE(:startDate, 'MM/DD/YYYY')) AND TO_DATE(START_DATE, 'MM/DD/YYYY') <= TO_DATE(:endDate, 'MM/DD/YYYY')) AND (LOWER(DEALER) LIKE LOWER( :dealer )  || '%') AND ((:status = 'Active' AND INACTIVE_DATE IS NULL) OR (:status = 'Inactive' AND INACTIVE_DATE != ' ')) ORDER BY DEALER, DICE">
         <SelectParameters>
