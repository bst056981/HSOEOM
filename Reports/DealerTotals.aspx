...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    ...
    <asp:SqlDataSource ID="dsCustomer" runat="server" ConnectionString="<%$ ConnectionStrings:ConnectionString %>"
        ProviderName="<%$ ConnectionStrings:ConnectionString.ProviderName %>" SelectCommand="SELECT DEALER, ACTIVE, INACTIVE FROM (SELECT DEALER, COALESCE(ACTIVE, 0) ACTIVE, COALESCE(INACTIVE, 0) INACTIVE FROM (SELECT DISTINCT '1' REC, A.DEALER, ACTIVE.CNT AS ACTIVE, INACTIVE.CNT AS INACTIVE FROM CUSTOMER A LEFT OUTER JOIN (SELECT DEALER, COUNT(DEALER) AS CNT FROM CUSTOMER WHERE INACTIVE_DATE IS NULL AND DEALER != 'CPRT' GROUP BY DEALER UNION SELECT DISTINCT DEALER, 113 AS CNT FROM CUSTOMER WHERE INACTIVE_DATE IS NULL AND DEALER = 'CPRT' GROUP BY DEALER) ACTIVE ON A.DEALER = ACTIVE.DEALER LEFT OUTER JOIN (SELECT DEALER, COUNT(DEALER) AS CNT FROM CUSTOMER WHERE INACTIVE_DATE != ' ' GROUP BY DEALER) INACTIVE ON A.DEALER = INACTIVE.DEALER UNION SELECT '2' AS REC, 'TOTAL' AS DEALER, (SELECT CNT + 113 FROM (SELECT COUNT(DEALER) AS CNT FROM CUSTOMER WHERE INACTIVE_DATE IS NULL AND DEALER != 'CPRT' AND DEALER != 'DICE' AND DEALER != 'IPRCV' AND DEALER != 'NMBURG' AND DEALER != 'NMFIRE' AND DEALER != 'PART' AND DEALER != 'SVC' AND DEALER != 'TEST' AND DEALER != 'UNKNO')) ACTIVE, (SELECT COUNT(DEALER) FROM CUSTOMER WHERE INACTIVE_DATE != ' ' AND DEALER != 'DICE' AND DEALER != 'IPRCV' AND DEALER != 'NMBURG' AND DEALER != 'NMFIRE' AND DEALER != 'PART' AND DEALER != 'SVC' AND DEALER != 'TEST' AND DEALER != 'UNKNO') INACTIVE FROM CUSTOMER GROUP BY 1 ) ORDER BY REC, DEALER ) WHERE DEALER != 'DICE' AND DEALER != 'IPRCV' AND DEALER != 'NMBURG' AND DEALER != 'NMFIRE' AND DEALER != 'PART' AND DEALER != 'SVC' AND DEALER != 'TEST' AND DEALER != 'UNKNO'"></asp:SqlDataSource>
